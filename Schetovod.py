...
class MyApp(App):
    ...
    def build(self):
        while True:
            ...
            self.schetovod = Label(text = "Счетовод", font_size = 150, color = (255,1,1,1))
            bl.add_widget( self.schetovod )
            self.lblp = Label(text=e, font_size = 120)
            bl.add_widget( self.lblp )

            self.lbl = Label(text=self.formula, font_size = 250)
            self.yes = Label(text=self.labelO, font_size = 75, color = (0,1,0,1))
            self.no = Label(text=self.labelX, font_size = 75, color = (1,0,0,1))
            ...
            anim = Animation(x=0, y=1400)
            anim.start(self.schetovod)
            anim = Animation(x=0, y=1000)
            anim.start(self.lblp)
            anim = Animation(x=0, y=450)
            anim.start(self.lbl)
            anim = Animation(x=-400, y=1600)
            anim.start(self.yes)
            anim = Animation(x=-400, y=1500)
            anim.start(self.no)
